...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" version="1.0" xmlns:mssqlcrt="urn:sql-microsoft-com:sqlcrt" xmlns:mssqldbg="urn:sql-microsoft-com:sqldbg">
    ...
    <xsl:param name="post-select-query-hint">--SQL generated using EXASolution cartridge for SSAS 2008 (10.0)</xsl:param>


  <!-- SQL Server 10.0 pluggable cartridge                              -->
...
<mssqlcrt:provider type="prefix" managed="yes">EXASolution</mssqlcrt:provider>
  ...
  <mssqlcrt:schema-classes>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				SELECT  <xsl:apply-templates select="./Distinct"/>
					...
					<xsl:if test="count(../Phase) != 0">, COUNT(*) AS [COUNT_7673aff6-2445-4ef6-a4c9-7bf3d93bd42a] </xsl:if>
	...
	<xsl:template match="Create">
		...
		<xsl:choose>
			<xsl:when test="number($phase)=1">
				VIEW <xsl:apply-templates select="./View"/> ( <xsl:apply-templates select="./*[4]"/>, [COUNT_7673aff6-2445-4ef6-a4c9-7bf3d93bd42a] ) WITH SCHEMABINDING AS <xsl:apply-templates select="./*[5]"/>
			</xsl:when>
	...
	<xsl:template match="Having">
		HAVING <xsl:call-template name="print-children-list"/>
	</xsl:template>
	...
	<xsl:template match="Type">
		...
		<xsl:choose>
			...
			<xsl:when test="$type-val = 'DBTYPE_I1'">      Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_I2'">      Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_I4'">      Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_I8'">      Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_UI1'">     Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_UI2'">     Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_UI4'">     Integer      </xsl:when>
			<xsl:when test="$type-val = 'DBTYPE_UI8'">     Integer      </xsl:when>
			...
			<xsl:when test="$type-val = 'DBTYPE_GUID'"> varbinary  </xsl:when>
