...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" version="1.0" xmlns:mssqlcrt="urn:sql-microsoft-com:sqlcrt" xmlns:mssqldbg="urn:sql-microsoft-com:sqldbg">
	...
	<xsl:param name="post-select-query-hint">--SQL generated using EXASolution cartridge for SSAS 2014 (12.0)</xsl:param>
  ...
  <mssqlcrt:parameter-style native="unnamed" managed="named"/>
	...
	<mssqlcrt:capabilities>
        ...
        -->
		<mssqlcrt:limit-table-identifier-length>255</mssqlcrt:limit-table-identifier-length>
		<mssqlcrt:limit-column-identifier-length>255</mssqlcrt:limit-column-identifier-length>
...
<xsl:template match="Sources">
  FROM <xsl:call-template name="print-children-list"/>
	...
	<xsl:template match="DatePart">
		UTILS.DATEPART( <xsl:apply-templates select="./*[1]"/> , <xsl:apply-templates select="./*[2]"/> )
	</xsl:template>
	...
	<xsl:template name="print-non-parametric-parameter">
		...
		<xsl:choose>
			...
			<xsl:when test="$db-type = 'DBTYPE_BOOL'">CAST(<xsl:value-of select="$value"/> AS boolean)</xsl:when>
